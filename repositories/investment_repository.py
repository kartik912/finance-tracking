...
class InvestmentRepository(BaseRepository[Investment]):
    ...
    def insert(self, entity: Investment) -> Investment:
        ...
        try:
            ...
            get_bus().publish(Events.INVESTMENT_WRITE, {"id": entity.id})
            return entity
        except Exception:
            session.rollback()
            raise
    ...
    def update(self, entity: Investment) -> Investment:
        ...
        try:
            ...
            get_bus().publish(Events.INVESTMENT_WRITE, {"id": merged.id})
            return merged
        except Exception:
            session.rollback()
            raise
    ...
    def delete(self, entity_id: int) -> bool:
        ...
        try:
            ...
            session.delete(obj)
            session.commit()
            get_bus().publish(Events.INVESTMENT_WRITE, {"id": entity_id})
            return True
        except Exception:
            session.rollback()
            raise
